...
```
可以使用这个方法关闭通道，
4.在一个发送端多个接收端的情况下，先用Waitgroup的Add数来控制通道的大小，然后一个发送端开始发送数据，并且设置好停止发送的条件，并在不想发送的时候停止关闭通道。多个接收端开始接收，并且每一次接收完毕就waitgroup.Done。这样做就可以确保全部接受，并且符合不在接收端关闭通道的原则。
